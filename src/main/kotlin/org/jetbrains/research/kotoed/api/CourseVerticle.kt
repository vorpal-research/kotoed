...
class CourseVerticle : AbstractKotoedVerticle(), Loggable {
    ...
    suspend fun handleCreate(course: CourseRecord): DbRecordWrapper {
        ...
        eb.send(Address.DB.verify(course.table.name), res.toJson())
    ...
    suspend fun handleRead(course: CourseRecord): DbRecordWrapper {
        ...
        val status: VerificationData = vxa<Message<JsonObject>> {
            eb.send(Address.DB.verify(course.table.name), res.toJson(), it)
        }.body().toJsonable()
    ...
    suspend fun handleError(verificationData: VerificationData): JsonObject =
            verificationData.errors
                    .map { selectById(Tables.COURSE_STATUS, it, CourseStatusRecord::class) }
                    .map { it.toJson() }
